...
for _, row in interact_df.iterrows():
    key = frozenset([
        str(row["drug1"]).strip().lower(),
        str(row["drug2"]).strip().lower()
    ])
    interaction_map[key] = {
        "severity": str(row.get("severity", "moderate")).lower(),
        "description": str(row.get("description", "Potential interaction"))
    }
    ...
    interaction_map = {}
for _, row in interact_df.iterrows():
    key = frozenset([str(row["drug1"]).strip().lower(),
                     str(row["drug2"]).strip().lower()])
    interaction_map[key] = {
...
def ml_predict(drug1, drug2):
    ...
    x = np.array([[ 
        le1.transform([cat1])[0] if cat1 in le1.classes_ else 0,
        le2.transform([cat2])[0] if cat2 in le2.classes_ else 0,
        0, 0, 0, int(cat1 == cat2)
    ]])
...
def check_interactions(drugs):
    ...
    for d1, d2 in combinations(drugs, 2):
        key = frozenset([d1.lower(), d2.lower()])
        ml_sev, ml_conf, _ = ml_predict(d1, d2)
